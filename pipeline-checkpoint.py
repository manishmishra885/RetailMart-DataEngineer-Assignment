...
def task2_clean_data(df_sales):
    ...
    df_sales["sale_date"] = pd.to_datetime(df_sales["sale_date"])   # date format
...
def task4_load_to_db(df_merged):
    ...
    df_merged.to_sql("retail_sales", con=engine, if_exists="replace", index=False)
    ...
    with sqlite3.connect("retailmart.db") as conn:
        top3_products = pd.read_sql_query(query_top3, conn)
